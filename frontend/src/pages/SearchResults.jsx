...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL}/products/search`, { params: { q: query } })
      .then(res => {
        const results = res.data || [];
        setProducts(results);
        if (results.length > 0) {
          const highest = Math.max(...results.map(p => p.price));
          setMaxPrice(highest);
          setValue([0, highest]);
        }
        setLoading(false);
  ...
  return (
    <div className="collections-page">
      ...
      <div className="coll-title-section">
        <h1 className="coll-page-title">SEARCH RESULTS</h1>
        <p className="coll-breadcrumb">
      ...
      <div className="px-3 px-md-4 px-lg-5">
        ...
        <div className="coll-filter-info">
          <span className="coll-count">{filtered.length} Products Found for "{query}"</span>
          ...
          <div className="coll-grid">
            {filtered.length > 0 ? filtered.map(product => (
              ...
              <div className="text-center mt-5" style={{gridColumn:'1/-1'}}>
                <i className="fa-solid fa-search" style={{fontSize:'48px', color:'#ddd', display:'block', marginBottom:'16px'}}></i>
                <h5>No products found for "{query}"</h5>
                <p className="text-muted">Try different keywords or browse our collections.</p>
                <Link to="/collections" className="btn btn-dark mt-3">Browse Collections</Link>
              </div>
            )}
          </div>
        )}
      ...
      <Offcanvas show={show} onHide={() => setShow(false)} placement="start">
        ...
        <Offcanvas.Body className="coll-offcanvas-body">
          ...
          <div className="coll-filter-section">
            ...
            <p className="coll-price-range">₹{value[0].toLocaleString('en-IN')} — ₹{value[1].toLocaleString('en-IN')}</p>
          </div>
          ...
          <div className="coll-filter-section">
            ...
            <ul className="list-unstyled coll-type-list">
              <li><label><input type="checkbox" checked={selectedCategories.length === 0} onChange={() => setSelectedCategories([])} /> All Products</label></li>
              {categories.map(cat => (
                <li key={cat._id}>
                  <label>
